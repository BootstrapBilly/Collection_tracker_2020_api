...
exports.add_book = async (req, res, next) => {
    ...
    if (book_exists) return res.status(409).json({ error: "Book already exists with that year and condition. Only 1 copy of each condition is allowed for each year."})

    const new_book = new Book({year:year, condition:condition} )//create a new book with the given input
    ...
    book_saved ? res.status(201).json({message: "Book saved successfully", success:true}) : res.status(500).json({error:"Something went wrong with the database"})
